...
export type ConversationEventHandler = (event: ElevenLabsWebSocketEvent) => void;
    ...
    try {
      ...
      switch (data.type) {
        ...
        case "user_transcript":
          ...
          console.log(`⏱️ [${new Date().toISOString()}] ELEVENLABS_USER_TRANSCRIPT: "${userEvent.user_transcription_event.user_transcript}"`);
          this.handlers.onUserTranscript?.(userEvent.user_transcription_event.user_transcript);
          this.handlers.onUserSpeaking?.(false); // User stopped speaking
          break;
          
        case "agent_response":
          const responseEvent = data as AgentResponseEvent;
          console.log(`⏱️ [${new Date().toISOString()}] ELEVENLABS_AGENT_RESPONSE: "${responseEvent.agent_response_event.agent_response}"`);
          // This is the key part - forward agent response to HeyGen puppet
          this.handlers.onAgentResponse?.(responseEvent.agent_response_event.agent_response);
          break;
        ...
        case "audio":
          ...
          this.handlers.onAudioResponse?.(audioEvent.audio_event.audio_base_64, audioEvent.audio_event.event_id);
          break;
  ...
  private startAudioStreaming() {
    ...
    try {
      ...
      this.mediaRecorder.start(50); // 50ms chunks for lower latency
        ...
        console.error("❌ Fallback audio streaming also failed:", fallbackError);
      }
    }
  }
